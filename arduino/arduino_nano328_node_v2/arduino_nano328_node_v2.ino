...
void Car::initEncoders() {
  ...
  attachInterrupt(digitalPinToInterrupt(ENC_FR_PIN), incFRCallback, CHANGE);
  attachInterrupt(digitalPinToInterrupt(ENC_FL_PIN), incFLCallback, CHANGE);
  attachInterrupt(digitalPinToInterrupt(ENC_BR_PIN), incBRCallback, CHANGE);
  attachInterrupt(digitalPinToInterrupt(ENC_BL_PIN), incBLCallback, CHANGE);
}

void Car::initRCInput() {
  pinMode(THROTTLE_PIN, INPUT_PULLUP);
  pinMode(STEERING_PIN, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(THROTTLE_PIN), calcThrottleCallback, CHANGE);
  attachInterrupt(digitalPinToInterrupt(STEERING_PIN), calcSteeringCallback, CHANGE);
}
